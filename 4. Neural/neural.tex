...
The instigator of that paradigm shift was the transformer architecture~\cite{vaswani2017attention}, which by now enjoys an unprecedented \hide[popularity]{pop-science status (saving me the hassle of having to regurgitate yet another ``transformers explained'' pamphlet)}.
...
\hide[In what follows, I describe]{[Rather than suffocate us all with irrelevant distractions and boring tables of deltas, I think it's best if I stick to describing]} in detail only the current and most recent implementation~\cite{spindle}, drawing parallels with isolated historical insights only when relevant.
