...
Rather than suffocate us all with irrelevant distractions, I think it's best if I stick to describing in detail only the current and most recent implementation which rests on the supertagger of Section~\ref{subsection:gc}, drawing parallels with isolated historical insights only when relevant.
...
Initial experiments on \AE thel $\mathtt{0.4}$ showed that a beam width of 2 would suffice to bring parsability (i.e. invariance correctness) to 100\% and coverage (i.e. \textit{some} parse obtained) to 95.3\% (up from 87.7\% and 89.9\%, respectively), with tangible but diminishing returns to accuracy from further increasing the beam width up to 5.
...
This is a cheap and efficient way to selectively allocate weight on (some of) the decoder's representations, partially disentangling the two problems.
...
Leaf representations are binned according to their sentential index, atomic type and polarity (e.g. a single bin would be all occurrences of a positive $\type{np}$ in sentence \#13 of the input batch).
...
The joint architecture takes longer to train than the standalone tagger, owing to the slower forward and backward passes, but also an increased number of epochs required to reach convergence.
The linking module is surprisingly fast to converge, but its inclusion is detrimental to the supertagging component, as its loss term dominates the sum early on.
Since accurate supertagging is a prerequisite to parsing, linking loss is scaled by 10\% to promote smoother training curves and a better task balance.
To nudge the model away from local optima and diffuse any gradient conflict, one of the two losses is occassionally zeroed out with a 20\% chance~\cite{woo2021revisiting}.

\paragraph{Evaluation}

%Regardless of how leaf representations are sourced, the high-level pipeline follows the same general principles.
%First, the supertagger's output is ``parsed'' into types proper.
%Assuming no structural integrity issues, leaf positions are then indexed, keeping track of their polarities and atomic types -- the result is a proof frame.
%For the frame to be an eligible starting point for proof search, it must satisfy the count invariance property, requiring an equal count of positive and negative leaves for every atomic type present.
%Assuming it does, leaf representations are extracted and binned according to their type and polarity (also sentential index, when processing in batches).
%Each bin is zipped with its inverse polarity counterpart, and their element-wise agreement scores are computed via some similarity metric.%
%	\footnote{Note that the zipping function is not \textit{the} similarity metric but the pairwise application of one. 
%	In other words, the agreement scores are independently computed between pairs of elements from the two sets.
%	This is necessary to account for the variably sized sets encountered without ad-hoc padding, but also to ensure that the end operator is permutation invariant.}
%The resulting matrices are finally normalized by a fixed number of Sinkhorn iterations, and sent on their way.
%

%%%To procure these activations, three Sinkhorn iterations are applied to the agreement scores computed in parallel
%%Dropout is applied to the intermediating weight vector to randomly zero out the same pairwise interactions in each batch.
%%%Three Sinkhorn iterations suffice for results to sharpen the matrices into being almost binar
%%%
%%
%%%Dropout is applied to the intermediating weight vector
%%
%%\paragraph{Inference}
...
%%\paragraph{Overview}
%%In their first public appearance, neural proof nets were seen hand in hand with a variation of the symbol sequential transformer of Section~\ref{subsection:snmt} -- this was less of a conscious choice and more of a lack thereof.
%%The language mdo underlying the transformer was originally substituted for BERTje and eventually for RobBERT~\cite{delobelle2020robbert}.
%%To account for the missing right context of the decoder's output (it being left-to-right autoregressive), leaf representations were processed by a second encoder, used to cross-attend between the full input and the entire decoded output, contextualizing the latter based on the former.
%%Negative and positive representations were processed by distinct feed-forward networks, and their agreement scores would be computed as their dot-products.
%%A finite number of Sinkhorn steps would then process the output, which would be discretized by rounding during inference.
%%Boring and irrelevant details aside, 
%
%% only the current implementation 
%%The new supertagger of Section~\ref{subsection:gc} presents us with an alternative that looks like a no brainer at a first glance.
%%Alas, the truth is more complicated; the choice of supertagger has a multidimensional effect on the system's end-to-end performance.
%
%%To quantify the effect of this, we will go through three distinct evolutionary phases of neural proof nets through time.
%
%
%%\subsubsection{Symbol Sequential}
%%In the original experiments of~\citet{kogkalidis-etal-2020-neural}, the supertagger consisted of a 



%\subsection{Proof Nets in \NLPplus}
%Before we lay out our schemes and proceed with our machinations, we need to take a brief pause for the sake of formal transparency.
%The use of proof nets as our representational standard might be seen as implicating an equivalence between proofs and proof nets.
...
%There's a whole lot of proof patterns we don't expect to ever encounter, and can thus safely ignore (e.g. box introduction rules, diamonds in result position, box-diamond chains etc., just to name a few).
%This might still seem quite ambituous at a first glance -- proof nets have no means of encoding the positionally underspecified diamond elimination rule $\diamond E$, and are altogether unperceptive to the structural extraction rule $\Extraction$
%This would've indeed been the case if it wasn't for our providence: we have imposed a canonical placement on both these rules, which should relieve the burden of translation.
%%If historicity is your
