...
The sequential structure can be seen as a fully connected directed graph, nodes being states (words) and edges labeled by relative distances between word pairs.
...
\begin{equation}
\tilde{a}_{i,j} = d_w^{-1/2}~(W_q \tilde{h}^\tau_i \odot W_r\mathcal{E}^w_{i,j}) \cdot W_k\tilde{h}^\tau_j
\end{equation}
...
The end-product bears semblance and owes credit to various contemporary lines of work.
From the architectural angle, it is as an application-specific offspring of weight-tied architectures, dynamic graph convolutions and structured self-attention networks.
...
The French TLGbank is \AE thel's distant but cherished relative. 
...
The new symbols correspond to notational and temporal shortcuts for multiple decisions compressed in a single time step, making for an unambiguous and invertible representational translation at the cost of an enlarged alphabet.%
	...
	Some of the composite alphabet symbols may appear in the test set (or in the wild) without having ever appeared in the training set -- these would be impossible to predict, even in the constructive setting!
...
We train using AdamW~\cite{loshchilov2018fixing} and a variable learning rate scaled by a linear warmup and cosine decay schedule over 25 epochs, scaled by 10\% for the encoder.
During training we provide strict teacher forcing and apply feature and edge dropout at 20\% chance.
Same as before, the loss signal is derived as the label-smoothed negative log-likelihood between the network's prediction and the ground truth label.
Base-sized BERT variants are procured from the transformers library~\cite{wolf-etal-2020-transformers}: RoBERTa for English~\cite{liu2019roberta}, BERTje for Dutch~\cite{de2019bertje} and CamemBERT for French~\cite{martin-etal-2020-camembert}, all fine-tuned during training.

\paragraph{Evaluation}

%\begin{table*}[h]
%%    \newcolumntype{N}{>{\centering\arraybackslash}p{0.089\textwidth}}
%    \newcolumntype{L}{>{\raggedright\arraybackslash}X}
%    \newcommand{\num}[1]{\multirow{1}{*}{#1}}
%    \newcommand{\ave}[2]{\multirow{1}{*}{~~#1}{\textsubscript{$\pm$#2}}}
%
%    \centering
%    {\smaller
...
%    \toprule
%    \multicolumn{7}{l}{\textit{\textbf{CCG (original)}}} \\
%    & \multicolumn{1}{l}{Symbol Sequential LSTM /w n-gram oracles\\
%	& \multicolumn{1}{l}{~\cite{liu2021generating}}
%    & 95.99 & 96.40 & 65.83 & 
%    \multicolumn{2}{c}{8.65\textsuperscript{!}} \\
%    & \multicolumn{1}{l}{Symbol Sequential LSTM
%    {~\cite{bhargava-penn-2020-supertagging}}}
%    & 96.00 & -- & -- & -- & $\sim$\textbf{5}\\
%    & \multicolumn{1}{l}{Cross-View Training
%    {~\cite{clark-etal-2018-semi}}} 
%    & 96.10 & -- & -- & -- & n/a \\ 
%    & \multicolumn{1}{l}{{Recursive Tree Addressing}
%    {~\cite{prange-etal-2021-supertagging}}}
%    &  \num{96.09} & \num{96.44} & \num{68.10} & \num{\textbf{37.40}} & \num{\textbf{3.03}} \\
%    & \multicolumn{1}{l}{{BERT Token Classification}
%    {~\cite{prange-etal-2021-supertagging}}}
%    & \num{96.22} & \num{\textbf{96.58}} & \num{70.29} & \num{23.17} & n/a\\
%    & \multicolumn{1}{l}{Attentive Convolutions
%    {~\cite{tian-etal-2020-supertagging}}} 
%    & \textbf{96.25} & \textbf{96.64} & 71.04 & n/a & n/a\\ 
%    \addlinespace
%    & \multicolumn{1}{l}{{Heterogeneous Dynamic Convolutions}
%    {~(this work)}} 
...
%    \addlinespace
%    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%    \multicolumn{2}{L}{\textit{\textbf{CCG (rebank)}}} \\
%    & \multicolumn{1}{l}{{Symbol Sequential Transformer}\textsuperscript{\textdagger}
%    {~\cite{kogkalidis-etal-2019-constructive}}}
%    & \num{90.68} & \num{91.10} & \num{63.65} & \num{34.58} & \num{\textbf{7.41}} \\
%    & \multicolumn{1}{l}{Symbol Sequential LSTM\textsuperscript{\textdagger}
%    {~\cite{bhargava-penn-2020-supertagging}}}
%    & 93.92 & 94.39 & 65.48 & 19.00 & 0.00\\
...
%    {~\cite{prange-etal-2021-supertagging}}}
%    &  \num{94.70} & \num{95.11} & \num{68.86} & \num{\textbf{36.76}} & \num{4.94} \\
%    & \multicolumn{1}{l}{{Token Classification}
%    {~\cite{prange-etal-2021-supertagging}}}
%    &  \num{94.83} & \num{95.27} & \num{68.68} & \num{23.99} & \num{n/a} \\
%    \addlinespace
%    & \multicolumn{1}{l}{{Heterogeneous Dynamic Convolutions}
...
%    {~\cite{moot2019}}}
%    & \num{93.20} & \num{95.10} & \num{75.19} & \num{25.85} & \num{n/a}\\
%    & \multicolumn{1}{l}{{BERT Token Classification\textsuperscript{\textdaggerdbl}}}
%    & \num{\textbf{95.93}} & \num{\textbf{96.44}} & \num{81.39} & \num{47.45} & \num{n/a}\\
%    \addlinespace
%    & \multicolumn{1}{l}{{Heterogeneous Dynamic Convolutions}
%    {~(this work)}}
...
%    & \multicolumn{1}{l}{{Heterogeneous Dynamic Convolutions}
%    {~(this work)}}
%    & \ave{\textbf{93.67}}{0.04}
%    & \ave{\textbf{94.72}}{0.13}
%    & \ave{\textbf{73.45}}{0.46}
%    & \ave{\textbf{53.83}}{1.14}
%    & \ave{15.79}{1.32} \\
%    \bottomrule
%    \addlinespace
%    \multicolumn{7}{l}{{\fixh{\textsuperscript{!}}%
...
%    \multicolumn{7}{l}{{\fixh{\textsuperscript{\textdaggerdbl}}Our replication.}}\\
%    \multicolumn{7}{l}{{\textsuperscript{\textborn}Model trained and evaluated on an older dataset version and tree sequences spanning less than 140 nodes in total.}}
%    \end{tabularx}}
...
%During inference, each next fringe can be easily generated with minimal structure manipulation.
%Decoded symbols that correspond to tree-forming operators are isolated, and their indices are multiplied by two (to create the positions of their left children), and then offset by one (to create the positions of their right children). 
%Interleaving the offset and non-offset vectors yields the positions of both descendants of a single binary.
%In the same vein, the new state indices are simply the repetition of their respective ancestor indices.
%Positional embeddings of the current fringe are kept and reused in the ensuing node feedback phase (zero waste).
