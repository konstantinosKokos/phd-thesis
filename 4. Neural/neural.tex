...
\chapabstract{Like playing tic-tac-toe, but with types}
...
The end-goal not just theoretically noble, but also technically relevant; distributional semantics and word vectors were in their heyday, machine learning was rapidly taking off, parsers all of a sudden were becoming reliable.
...
But, unbeknownst to all, the forthcoming era in the field's evolutionary history would actually turn out to also be the dumbest (yet).
The advent of data efficient neural architectures -- combined with their immense potential for commercial applications and its allure to big tech -- brought large language models into the game.
...
Specifically in the natural language domain, advancing the conversion of text into formal representations is promoting accountable automation of textual processing, and eliminates the anthromorphic delusion of the ghost in the machine: to succesfully imitate linguistic form is not to understand meaning.
...
This is especially the case if ``almost correct'' is almost as good as correct, or the problem being modeled is intractable, making an approximation the best we could hope for.
...
In opting for a machine learning solution, one assigns more faith to a generic data cruncher in pretending to solve a problem than to oneself in designing a solution to that problem.
...
Lexicalized formalisms whose lexica are too eloquent are condemned to a premature end in fear of never making it past the supertagging phase, while it is only through such formalisms we could ever hope to trivialize the parsing process and fulfill the paradigm's ambition...
...
Adhering to evidence that pretrained language models seem to benefit either side of the encoder-decoder pipelne, the encoder would consist of a Dutch version of ELMo, the de facto language model at the time~\cite{peters-etal-2018-deep,che-etal-2018-towards}.
...
Decisions cannot be detached from the structure they bind to and help form -- to make a choice requires knowing your options, which in turn requires that previous choices were not just abstractly ``made'' but concretely evaluated.
...
Proof nets, on the other hand, are the embodiment of data parallelism, the most saught out property of neural computation and the holy grail of efficient training and optimization.
...
Second, a bijection between atomic formulas of inverse polarities, called axiom links.
...
At this point, the customary first reaction is to nervously contemplate all past decisions that led to this moment.
Neural models are notoriously ill-fit for discrete and combinatorial optimization -- what are we even doing?
The thought does briefly cross the mind: maybe we'd be better off trying some run-of-the-mill shift-reduce or chart-based parser with neurally guided decisions?
Nah.
...
Now, rather than look for the correct bijection between $P$ and $N$, we should be looking for the correct bijections between sets of the same ``type'' and inverse polarity.
...
It is not an unreasonable one though; the format is compact, and imports at least some of the structural constraints, faithfully capturing definition of a proof structure.
...
To apply in the setup envisaged here, all we need to do is assemble matrices containing unnormalized similarity scores in the cartesian product of positive $P_{\chi}$ and negative $N_{\chi}$ occurrences of formula tree leaves, one such matrix $S_{\chi}$ per unique atomic proposition $\chi$ present in a frame.
...
If training goes according to plan, the $\pi_{\chi}$-image (resp. $\pi_{\chi}^{-1}$) of each element of $P_{\chi}$ (resp. $N_{\chi}$) will outrank all competing items in $N_{\chi}$ (resp. $P_{\chi}$), i.e. be the largest entry of its row (resp. column) of $S_{\chi}$.
...
What we have in our hands is quite literally the world's tiniest parser.

%Initial experiments on \AE thel $\mathtt{0.4}$ showed that a invariance-adjusted beam width of 2 would suffice to bring parsability (i.e. invariance correctness) to 100\% and coverage (i.e. \textit{some} parse obtained) to 95.3\% (up from 87.7\% and 89.9\%, respectively), with tangible but diminishing returns to accuracy from further increasing the beam width up to 5 -- even though the exact numbers are no longer relevant, they paint an accurate picture of its effect.
...
Each bin is zipped with its inverse polarity counterpart, and their element-wise similarity scores are computed via the chosen metric.%
...
The joint architecture takes longer to train than the standalone tagger, owing to the slower forward and backward passes, but also due to the increased number of epochs required to reach convergence.
...
We proceed with evaluation with no training wheels: no pre- or post- processing, no tokenization or chunking oracles, and no length, depth or frequency-based thresholding.
...
On the bottom left, accuracy corresponds to the proportion of sentences assigned a proof that satisfies \textit{strict syntactic equality} to the ground truth one, and stands at a 55.63\%.
...
It also goes to show that the permutation strategy is incredibly robust; 97.95\% of the parsable sentences are \textit{actually} parsed, with the remaining 2.05\% of the errors being due to a structural link error.
...
\begin{table}
	\centering
	\begin{tabular}{@{}lcccc@{}}
		\textbf{modulo}			& $p$ & $r$ & $F_1$ & sentential accuracy\\
		\toprule
		--						& \stat{89.36}{0.05} & \stat{89.46}{0.06} & \stat{89.17}{0.06} & \stat{55.63}{0.55}\\
		~modalities				& \stat{90.82}{0.02} & \stat{90.94}{0.03} & \stat{90.68}{0.03} & \stat{56.25}{1.02}\\
		~functional types 		& \stat{90.91}{0.03} & \stat{91.02}{0.04} & \stat{90.72}{0.04} & \stat{57.39}{0.71}\\
		~types					& \stat{92.14}{0.01} & \stat{92.30}{0.01} & \stat{92.00}{0.02} & \stat{58.83}{0.63}\\
	\end{tabular}
...
Relaxation one targets the functional core of the logic, applying a forgetful transformation to strip proofs of their modalities to examine typed function-argument structures in isolation.
Relaxation two targets the modal enhancement of the logic, collapsing the set of atomic types into a single point (thus treating all functional types of the same \textit{shape} as equal) to examine dependency structures in isolation.
Relaxing on both axes at once is essentially casting proofs into the untyped $\lambda$ calculus, where all we care about are the type- and dependency- agnostic function/argument structures.%
...
The results are averaged over covered samples%
	\footnote{Averaging over the full test set would artificially deflate scores, since no partial proofs are returned from failing samples.}
and presented in Table~\ref{table:relaxations}.
Whether they are informative or not is up to debate; practically, they suggest that allowing for the occassional error in an atomic type or modality, about 92\% of the subproofs returned are correct, and about as many of the gold standard subproofs are returned, which in turn suggests that erroneous parses are likely the product of isolatd, local errors.
...
In trivializing the difficulties associated with hypothetical reasoning, they bring explicit variable binding into \textit{practical} relevance for wide-coverage categorial grammars, discrediting all the naysaying.
...
But doing so comes at the heavy price of a unidirectional data flow that lacks cross-component feedback; even though tagger and parser share the same representations, there's no communication from the latter to the form.
...
\subsection{Bonus: Proof Nets in \NLPplus}
...
Luckily, we are not really demanding an isomorphism between \NLPplus{} and its proof nets; we just want a locally one-to-one relation between the two, covering only the neighborhood of proofs inhabited by our linguistic usecases.
There's a whole lot of proof patterns we don't expect to ever encounter, and can thus safely ignore (e.g. diamonds in result position, box-diamond chains etc., just to name a few).
This might still seem quite ambituous at a first -- proof nets have no means of encoding the positionally underspecified diamond elimination rule $\diamond E$, and are altogether unperceptive to the structural extraction rule $\Extraction$
...
Our traversal starts as easy as it did before -- with two traversal directions.
...
Now if this all reads like black magic, it's probably because it is.
Obscure as it might be, it works: followed by $\eta$ and $\beta$ normalizations and $\alpha$ conversion, it produces a faithful back-and-forth translation for all the 55\,026 unique theorems of \AE thel.
The process goes as usual: start from the root of the conclusion in negative mode and hope for the best.
If you pass through all nodes and no type errors were thrown along the way, there's a pinky promise%
	\footnote{That's the closest we can do in lieau of a formal guarantee.}
...
Our original work~\cite{kogkalidis-etal-2019-constructive} on constructive supertagging has somewhat sensitivized the community to the problem of open-domain lexicalized parsing (despite citations thrown at blind attributing the idea to a second-tier and closed source derivative).
Despite the digressions against the paradigm, our results are inspired and preceded by a ton of inventive applications of generalized machine translation models repurposed for structure induction~\cite[inter alia]{vinyals2015grammar,wiseman2016sequence,dong-lapata-2016-language,buys2017robust}.
Serious steps forward include the ones by \citet{prange-etal-2021-supertagging} and \citet{Liu_Ji_Wu_Lan_2021}, who concurrently saught to account for the tree-like structure of lexical categories, the former through a tree-biased architecture and the latter through transition-based parsers applied at the lexical level.
...
The neuralification of proof nets is really just the creative application of modern breakthroughs in optimal transport learning and differentiable sorting~\cite[inter alia]{cuturi2013sinkhorn,mena2018learning,grover2018stochastic,peyre2019computational}.
The use of neural supertag representations for parsing (albeit at a more coarse level) is I believe due to~\citet{kasai-etal-2017-tag}.
\citet{moot2022perspectives} raises a shismatic criticism of proof nets from within%
	\footnote{You're lucky this isn't the communist party or the catholic church, Richard.}
, but also provides stimuli and incentives for alternative operationalizations.
