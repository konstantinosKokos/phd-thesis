...
Figure~\ref{figure:litten_acg_der} presents the simple applicative construction of the type previously assigned to \textex{litten}, $\diamond\Box (\np[s]\divleft\np[s])\divright\pp[s]$ (contexts are intentionally left empty and axioms replaced by abstract constants for brevity).
...
Elegant and successful as they might be in the meta-theoretical world, abstract categorial grammars have not fared as well with linguistic applications, in part due to their computationally intractable nature.
...
It seems therefore that the appealing simplicity and elegance of the tectogrammatic logic is counterbalanced by an increasingy bulky and cumbersome transition to the (equally simple, yet far less elegant) phenogrammatic logic (see also \citet{moot2014hybrid} for an extended discussion).
...
Type-logical grammars and abstract categorial grammars have monopolized our interest, yet are not the only members of the categorial grammar family.
...
In the strange region between type-logical grammars and abstract categorial grammars lives the beast of hybrid type-logical grammars.
Originally proposed in~\citet{kubota2012gapping}, hybrid type-logical grammars utilize a combination of the slash and backslash of traditional Lambek Calculi and the non-directional linear implication to give birth to a type grammar that combines the merits of both approaches while suffering the restrictions of neither.
