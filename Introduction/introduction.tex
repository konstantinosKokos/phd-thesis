...
\begin{center}
	...
	\end{tabularx}
\end{center}
As an example, applying $\hm{.}$ to the proof of Figure~\ref{figure:lovecract_rel_clause} yields the derivational term:
\[
...
% loop back to montague semantics
% continuations
% proof net

\subsection{Abstract Categorial Grammars}
\label{subsection:ACG}
The computational pipeline of the previous section is of sequential form; at its origin point is the syntactic calculus, from which derivational and then lexical semantics are iteratively obtained.
In that sense, type-logical grammars are syntax first, where syntax is meant to convey the hidden process that forms grammatically correct sentences.
This is not incompatible with the view of \citet{curry1961some}, who stipulated a two-layered hierarchy of grammaticality criteria.
The deep layer, called \textit{tectogrammar}, is one concerned solely with the well-typedness of grammatical function domains and the validity of their interpretations.
The shallow layer, called \textit{phenogrammar}, is where tectogrammatical proofs are transformed and cast to surface forms that abide by the linear order precedence and constituency restrictions imposed by the language.
There is no question of the legitimacy of this distinction: it should be clear that phenogrammar, in Curry's terms, is our syntactic logic, and tectogrammar is what we earlier referred to as derivational semantics.
The type-logical operationalization can be seen as Aristotelian in nature (or parsing-oriented).
It follows the path from observable evidence to latent variables: the surface string is perceived as the yield of a shallow proof, from which a deep proof is extracted.

The inverse operationalization, more Platonic in nature (or generation-oriented), follows the opposite path: it places phenogrammar at the top of the generative process, and models the information flow from deep and abstract to shallow and concrete.
This perspective is adopted by abstract categorial grammars~\cite{de2001towards} and their contemporary and closely related lambda grammars~\cite{muskens2001lambda}.
Both are tectogrammar-first formalisms that make extensive use of $\ILL_{\li}$ and the Curry-Howard isomoprhism to obtain tectogrammatical realizations and concrete semantics via homomorphic translations of the phenogrammatic parse.
\begin{center}
\begin{tikzpicture}
...
The focal point will be abstract categorial grammars, as they are closer in spirit to what is to come later.
...
For the phenogrammar to tectogrammar picture to be made evident, the distinguished type needs to be mapped to the string type $\smallprop{s} \mapsto \mathtt{str}$, forcing terms of the object language to evaluate to strings.%
\footnote{Note that, despite appearances, $\mathtt{str}$ is a first-order type $*\li *$ ($*$ some arbitrary primitive) so as to permit the view of string concatenation as function composition:
\[
+ := \lambda x_1^{\mathtt{str}}x_2^{\mathtt{str}}x_3^{*}.x_1~(x_2~x_3)
\]}
...
Despite their elegance and objective success in the meta-theoretical world, abstract categorial grammars have not fared as well with linguistic applications, in large part due to their computationally costly nature.
...
Even in the lexicalized setup, every word (abstract constant) needs to be assigned both an abstract type and a unique object term for every phenogrammatic behavior it exhibits; two lexical dimensions, compared to the one of vanilla categorial grammars -- gone is the adage of words carrying their combinatorics on their sleeves.
...
% blocking overgeneration?
% enforcing grammaticality
% shortcuts flat, beats the purpose
