...
It states that from a proposition $\smallprop{A}$ one can infer that very proposition (duh!).
...
In our case, we will make do with two modalities from temporal logic, the diamond $\diamond$ and the box $\Box$, that form a \textit{residuated pair}.
Their residuation properties can be formulated either (i) in the form of a type-level biconditional derivability relation:
\begin{center}
	\begin{tabularx}{0.35\textwidth}{@{}ccc@{}}
	$\diamond \prop{A} \vdash \prop{B}$ iff $\prop{A} \vdash \Box\prop{B}$
	\end{tabularx}
\end{center}
or (ii) their monotonic behavior
\begin{center}
	\begin{tabularx}{0.35\textwidth}{@{}ccc@{}}
	...
	\end{tabularx}
\end{center}
and their compositions, where $\diamond\Box(\_)$ is an \textit{interior} and $\Box\diamond(\_)$ a \textit{closure} operator:
\begin{center}
	\begin{tabularx}{0.35\textwidth}{@{}ccc@{}}
...
Note that the free variables of the resulting expression ($\caseof{\diaelim t}{x_i}{s}$) are the union of the free variables of \smallterm{s} and those of \smallterm{t}, with the exception of \smallterm{x_i}, which is \textit{bound} by the case construct.
...
Feel free to substitute $\li$ for a directional variant as needed.
...
In their original purpose as envisaged by Lambek, his calculi would find use as a universal system of \textit{grammatical} computation -- a perspective adopted and advanced into what has presently come to be known as type-logical grammars.
In a natural language setting, the original Lambek calculus $\LC$ would be the logic of \textit{strings}; it can faithfully portray the generation of natural language utterances, where arbitrary reordering is a destructive process that ruins coherence.
...
\begin{figure}
	...
	\begin{tabularx}{0.5\textwidth}{@{}r@{\quad::\quad}l}
		...
		opiate 			& $\smallnp\divright\smallnp$\\
		behold[s]		& $(\smallnp\divleft\smalls)\divright\smallnp$\\
		pour[ed] 		& $\smallnp\divleft\smalls$\\
		there 			& $\smallgtype{adv} := (\smallnp\divleft\smalls)\divleft(\smallnp\divleft\smalls)$\\
		by				& $\pp\divright\np$\\
	...
	\[
		\infer[\divleft E]{(\w{opiate}\sbind\w{oceans})\sbind(\w{poured}\sbind\w{there}) \vdash \s}{
			...
			\infer[\divleft E]{\w{poured}\sbind\w{there} \vdash \np\divleft\s}{
				\infer[\Lex]{\w{poured}: \np\divleft\s}{}
				&
				\infer[\Lex]{\w{there}: \smallgtype{adv}}{}
			}
		}
	\]
...
Formulas like the above are supplied by and grounded in the lexicon. 
We will employ the miniature mock-up of Figure~\ref{figure:toy_lambek_lexicon}, with the aid of which we can at last derive our very first linguistic derivations!
The purely appicative examples of Figure~\ref{figure:nl_applicative_examples} should be straightforward to decipher.
Do not be spooked by the seemingly new rule of inference, $\Lex$, simply meant to convey type assignments pulled from our lexicon.
Unlike the identity axiom $\Ax$, which instantiates hypothetical variables, $\Lex$ introduces lexical constants.
Lexical constants behave just like variables, except they are neither wantonly typed nor amenable to function abstraction.
...
The choice of $\NL$ over $\LC$ means that the priority of application reflects in two distinct judgements, each with its own proof, that differ in the bracketing structure of the composite $\np$ they contain:
\begin{center}
...
Despite their elegance and formal appeal, Lambek calculi in their pure form suffer from an aversion to anomalies, which natural languages tend to exhibit at a disproportionate degree. 
...
If you have not skipped any parts yet, your reward should now manifest as a premonition of what is to come: modalities to the rescue!

%Ever since pioneering work integrated them 
%Modalities play an integral role in the modern type-logical toolkit~\cite{moortgat1997categorial,moortgat1999constants}.
%Pioneering work would  ~\cite{moortgat1995multimodal,kurtonina1997structural}

% should probably foresee what what comes next: the time of the modalities is finally at hand.
%Opting for the cop out option


% typing tension, type ambiguity
% no ex nihilo
% OVERGENERATION => M
