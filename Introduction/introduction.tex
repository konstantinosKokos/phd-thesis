...
Given an enumerable set $\mathcal{V}$ of unique names for indexed variables, with elements \{\term{x_i, x_j, x_k, x_l, \dots}\}, and denoting arbitrary but well-formed terms with \term{s}, \term{t}, \term{u}, we will use $\term{s}: \prop{A}$ (or $\term{s}^{\smallprop{A}}$) to indicate that term \term{s} is of type \prop{A}.
...
The dual rule, $\ii I$, allows us to create (so-called anonymous) functions by deriving a result \smallterm{s} dependent on some hypothesized argument \term{x_i } which is then \textit{abstracted} over as \smallterm{\lambda x_i .s}.
...
Its dual, $\ii E$, gives us the option to identify the two coordinates of a tuple \smallterm{s} with variables \smallterm{x_i } and \smallterm{x_j}, when the latter are hypothesized assumptions for deriving some program \smallterm{t}.
...
\begin{tikzpicture}
	...
	\node[outer sep=10pt]	(nlp) at (0, 0) 	{$\Sigma := \logic{NL}_{\diamond,\Box}$};
	\node	(ss)  at (4.75, 0.5) 	{Derivational Semantics};
	\node[outer sep=10pt]  	(ill) at (4.75, 0) 	{$\Tau := \ILL_{\li}$};
	\draw[->] (nlp) -- node[above] {$\hm{.}$}  (ill) ;
	\node[outer sep=10pt] (target) at (9, 0)	{$?$};
...
In being tectogrammar-first, they diverge, however, in its operationalization.
...
In its original definition, an abstract categoral grammar consist of two instantiations $\Sigma$, $\Tau$ of $\ILL_{\li}$, and a map between them.
...
As a more down-to-earth example, let us sketch the outlines of an abstract categorial grammar tasked with the production of an end-to-end linguistic example.
First, we need to specify our two logics $\Sigma$ and $\Tau$.
For efficacy and simplicity, we can have $\Sigma$ coincide with the derivational semantics logic of the previous section.
The object logic $\Tau$, being responsible for the surface materialization of our derivational proofs, will need to be a logic of strings, and is thus populated by a single atomic type $\propcon^{\Tau} := \{ * \}$; all abstract atoms are then sent to $\mathtt{str}$, the alias of $* \li *$.
...
Abstract constants will be sent to object terms via $\theta_0$, each of which must contain a single occurence of a term constant, in order to respect lexicalism and resource transparency.
The story is summarized in Table~\ref{table:toy_acg_lexicon}.
Object types are ommitted for brevity -- simply substitute all atoms of the corresponding abstract types with $\mathtt{str}$ to obtain them.
...
Applying the homomorphic translation to the derivational terms \ref{equation:litten_derterm} already proves quite an endeavour; twelve reduction steps later, we are presented with a seemingly reasonable output (function-argument brackets substituted with indendation for legibility):
\begin{align*}
...
&\quad									\str{suns}\\
\bredstar \ & \term{\lambda x_i.x_i + \str{litten}\str{by}\str{suns}\str{that}\str{the}\str{eye}\str{may}\str{never}\str{behold}}\\
=: & \	\smallterm{s}
\end{align*}
Upon closer inspection, though, several issues pop up.
A natural question to ask first is what could possibly ever make this specific source term acceptable, but the tectogrammatically erroneous subject-abstraction below not?
\[
(*) \ \term{litten~(by~(that~(\lambda x_i .(may~(never~(behold~(the~eye) )))~x_i))~(suns))}
\]
What's more, the end-term $\smallterm{s}$ is of type $\mathtt{str}\li\mathtt{str}$, making it an ill-fit for the discontinuous application to the \textit{substring} \str{opiate}\str{oceans} nested within \str{opiate}\str{oceans}\str{poured}\str{there}.
For the tectogrammatical form to surface correctly, we would need to alter the derivational proof by either including non-lexical abstract constants, or adjusting the derivational recipe with functional black magic.
This not particularly hard to accomplish:
%\begin{align*}
%	\smallterm{s} &\Longrightarrow \smallterm{\lambda x_i.x_i~\smallterm{s}}\\
%	\smallterm{poured~there~(opiate~oceans)} &\Longrightarrow \smallterm{\lambda x_j.x_j~(poured~there~(opiate~oceans))}\\
%\end{align*}

%Despite their elegance and objective success in the meta-theoretical world, abstract categorial grammars have not fared as well with linguistic applications, in part due to their computationally costly nature.
...
%Even in the lexicalized setup, every word (abstract constant) needs to be assigned both an abstract type and a unique object term for every phenogrammatic behavior it exhibits; two lexical dimensions, compared to the one of vanilla categorial grammars.
%Enforcing grammaticality while blocking overgeneration of the object language similarly requires a careful finetuning of both the abstract language and the translation, in parallel --  gone is the adage of words carrying their combinatorics on their sleeves. 
...
%This last part could in principle be partially mitigated by flattening complex syntactic phenomena into lower-order (alias) types in the source domain, and outsourcing their expansion to a parallel grammar for concrete semantics -- this is less of a solution and more of a deferral, though.
