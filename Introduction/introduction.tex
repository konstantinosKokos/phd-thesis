...
To make the hypothesis movable, we need to instantiate it as a box -- for the pure function contained therein to be applicable, the box needs to be removed, enclosing the hypothesis in angular brackets, which in turn license its structural extraction to the rightmost edge of the assumptions.
...
\begin{figure}
	...
	\begin{subfigure}{1\textwidth}
		...
		\]
		\caption{Derivation using a hypothetical postnominal modifier..}
	...
	\end{subfigure}
	\caption{Long distance postnominal modification.}
	\label{figure:todo}
\end{figure}
...
There, the subordinate clause \textex{\_ can vary in its position} contains a gap in the subject position, which the head \textex{a gap} occupies implicitly.
This is not the case in the last relative clause \textex{which the head \textex{gap} occupies implicitly}, whose subordinate clause \textex{the head \textex{gap} occupies \_ implicitly} contains a non-peripheral (nested) gap in direct object position.
What a mess! 
...
\end{align*}
The second version launches a mobile \np[s] hypothesis via the same diamond-box pattern employed earlier.
Its use in deriving 
...
\begin{figure}
	...
	\begin{subfigure}{1\textwidth}
		\smaller
		\[
			\infer[\rulestyle{ass}_{\diamond} (x2)]{((\w{the}\sbind\w{eye})\sbind(\w{may}\sbind(\w{never}\sbind(\w{behold}))\sbind\bracket{\term{x_i}}\vdash \s}{
...
%\cite{moortgat1999constants}

%Their original 
%Ever since their initial integration with the multiplicative 

%~\cite{moortgat1997categorial,moortgat1999constants}.
