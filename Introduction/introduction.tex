...
The proof of Figure~\ref{figure:lovecract_rel_clause} employs this typing in combination with the $\rulestyle{ass}_{\diamond}$ rule to derive the object-relative clause \textex{that the eye may never behold} -- we can now use it to modify the \textex{suns} of Figure~\ref{figure:lovecract_rel_clause}, bringing us one step closer to success.
...
\begin{tikzpicture}
	...
	\node	(ss)  at (6, 0.5) 	{Derivational Semantics};
	\node[outer sep=10pt]  	(ill) at (6, 0) 	{$\Tau := \ILL_{\li}$};
	\draw[->] (nlp) -- node[above] {$\hm{.}$}  (ill) ;
\end{tikzpicture}
\end{center}

To make things concrete, let's consider this in the context of the source logic $\Sigma$ being identified with the instantiation of $\NL_{\diamond,\Box}$ of the previous section, and the target logic $\Tau$ being its $\ILL_{\li}$ mirror image.
We will denote with $\propcon^X$ the set of atomic types, of which $\types^X$ the inductive closure of well-formed types, and $\cons^X$ and $\vars^{X}$ the sets of constants (words) and variables, of which $\terms^X$ the inductive closure $\lambda$ of well-formed terms, using the superscript $X := \Sigma \ | \ \Tau$ to distinguish between the two logics.
...
On the type level, $\eta$ must specify a pointwise mapping $\eta_0$ from the propositional constants $\propcon^{\Sigma}$ of the source logic to types $\types^{\Tau}$ of the target logic.
...
\begin{table}[ht]
	...
	\begin{tabularx}{0.52\textwidth}{@{}c@{\quad$\mapsto$\quad}c@{}}
		...
		$\smallterm{x_i} \in \vars^{\Sigma}$				& $\theta_0(\smallterm{x_i}) := \smallterm{x} \in\vars^{\Tau}$\\
...
If this navigation between syntactic and semantic theories would be a naval voyage, the Curry-Howard isomorphism would be its North Star.
...
Each conjunct fulfills a descriptive filter that intersects the properties of its argument with the properties attributed by its internal meaning.
That is, of all objects of type $\smallprop{NP}$, the first modifier withdraws all but those lit by unseeable suns, wheres the second one withdraws all but those with weird entities in their whirlpools.
...
$\lambda \smallterm{x_1x_2x_3.x_1~(x_2~x_3)}$, so as to allow the iteration of the intersective modifiers.%
...
A further passage from $\ILL$ to $\IL$ would give access to non-linear semantics, permitting the resource duplication necessary for resolving ellipses, etc.
% its argument would need to be distributed along both its constituents.
%Such a task involves resource duplication, which is clearly beyond the abilities of our derivational logic, pointing towards a need for 
%The problem dissolves in a non-linear semantic domain, where \textex{and} could be assigned the semantic term
%\[
%	\lambda x_1^{e\ii e}x_2^{e\ii e}x_3^{e}.(x_1~x_3) \wedge (x_2~x_3)
%\]
%Upon however conducting another passage to $\IL_{\ii}$
%Each conjunct has an intersective meaning by attributing to \textex{opiate oceans}
%, where each conjunct fulfills a descriptive role.
%For the  reading to be extracted, \textex{and} must be seen as supplying a

% case in point


% loop back to montague semantics
% possibility of using bracketing order semantics
% continuations
% role of the lexicon
 
%We will focusprovide a brief introduction focusing on the key aspects of interest.
% proof net
% syntax-semantics
% lexical semantics
