...
\section{Dropping Commutativity}
There is only one structural rule left\footnote{Or is there?}; it is time for \Exchange{} to go.
Dropping structural commutativity gives us an \textit{ordered} type system, i.e. one where variables must be used exactly in the order they were provided.
If we were now to naively go about our business using $\ILL$ without commutativity, we would soon stumble upon a pitfall. 
Recalling the shape of the $\li E$ rule, we come to the realization that the functions carried over to this new type system are suddenly picky; they can only be applied to arguments to their right.
This should raise some red flags: we want our type system to be directional, but not uni-directional: where shall we look for the left-biased variant of the implication?
The answer is simple: the conflation between the two directions was natural, up until a moment ago; having them both explicit would not amount to much, since by \Exchange{} they would be interderivable.
...
The logic that does our newfound friend justice has come to be known as the Lambek Calculus~\cite{lambek1958mathematics}.
A careful reader will notice the chronological inconsistency of our presentational tour: the Lambek Calculus predates Linear Logic, despite being a refinement of its purely linear part!
Formulas will now abide by the following grammar:
\begin{equation}
\prop{A}, \prop{B}, \prop{C} ::= p \ | \ \prop{A} \li \prop{B} \ | \ \prop{A} \il \prop{B}
% \ | \ \prop{A} \otimes \prop{B} \ | \ \prop{A} \with \prop{B}
\tag{$\LC$}
\label{grammar:LC}
\end{equation}
Alternative presentations include multiplicative and/or additive conjunction and/or disjunction, but the interesting feature lies in the two implications.
The rules of this fragment are presented in Figure~\ref{figure:lambek_calculus_rules}.
The directed version of the implication arrow has slightly denotational semantics: it points from argument to conclusion, meaning that what used to be $\li$ now becomes $\il$ (it looks for an argument directly to its right to produce a result).
The new version of $\li$ is in reverse: it looks for an argument directly to its left.
The use of the word \textit{directly} is not meaningless prose.
Another interesting aspect of the Lambek Calculus is that it brings forth a notion of adjacency.
\todo

\begin{figure}
	\centering
	\centering
	\begin{tabularx}{0.95\textwidth}{@{}CC@{}}
		\multicolumn{2}{@{}c@{}}{$\infer[Ax]{\prop A \vdash \prop A}{}$}\\[1em]
		$\infer[\il E]{\Gamma, \Delta \vdash \prop B}{\Gamma \vdash  \prop{B} \il \prop{A} & \Delta \vdash \prop{A}}$
		& 
		$\infer[\il I]{\Gamma \vdash \prop{B} \il \prop{A}}{\Gamma, \prop A \vdash \prop B}$\\[1em]
		$\infer[\li E]{\Gamma, \Delta \vdash \prop B}{\Gamma \vdash \prop A & \Delta \vdash \prop{A}\li\prop{B}}$
		& 
		$\infer[\li I]{\Gamma \vdash \prop A \li \prop B}{\prop A, \Gamma \vdash \prop B}$\\[1em]
	\end{tabularx}
	\caption{todo}
