...
\begin{center}
\begin{tikzpicture}
	\node (ds)		at (0, 1.5)		{Phenogrammar};
	\node (dsl)		at (0, 1)		{$\Sigma := \ILL_{\li}$};
...
Its original definition has an abstract categorial grammar consist of two instantiations $\Sigma$, $\Tau$ of $\ILL_{\li}$, and a map between them.
...
%$
%	\mathrm{max}\left(\mathcal{O}(\smallprop{A}) \ | \ \smallterm{c}: \smallprop{A} \in \cons^{\Sigma}\right)
%$
%$
%	\mathrm{max}\left(\eta_0(p) \ | \ p \in \propcon^{\Sigma} \right)
%$

%This can be used to 
%Embedding each member of the Chomsky hierarchy into an abstract categoral grammar equivalent reveals an interesting pattern: the grammar's complexity 
%It turns out that embedding the Chomsky hierarchy in 

%Abstract categorial grammars stand out from the rest of the categorial family in not being lexicalized by default.
...
%Abstract constants may contain lexical items that make their way to the final derivation, or simply compositional recipes that leave no imprint whatsoever.

% elegant in modeling formal grammars, meta-language, computational hierarchy
% bulky for real linguistic applications, all the weight on the lexicon
