...
\begin{figure}
	...
	\begin{subfigure}{1\textwidth}
		...
		\multicolumn{3}{@{}c@{}}{$\infer[Ax]{\prop A \vdash \prop A}{}$}\\[1em]
		\multicolumn{2}{@{}c}{
		$\infer[\li E]{\Gamma, \Delta \vdash \prop B}{\Gamma \vdash \prop A \li \prop B & \Delta \vdash \prop B}$}
		& 
		$\infer[\li I]{\Gamma \vdash \prop A \li \prop B}{\Gamma, \prop A \vdash \prop B}$\\[1em]
		\multicolumn{2}{@{}c}{
		$\infer[\otimes E]{\Gamma,\Delta \vdash \prop C}{\Gamma \vdash \prop A \otimes \prop B & \Delta, \prop A , \prop B \vdash \prop C}$}
		&
...
Its reading is straightforward: if one were to use $\Gamma$ once to derive \prop{A} and once to derive \prop{B}, proceed by joining the two, and then make the choice to keep the left proof, there was no need for the second proof in the first place.
