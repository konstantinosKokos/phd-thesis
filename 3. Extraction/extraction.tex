...
\begin{figure}
	...
	\aethelcap
		...
		{\term{\con{7}~\diaintro{vc}(\con{10})~\diaintro{su}(\boxelim{mod}(\boxelim{mod}(\con{4})~\con{5})~\con{6})}}
...
%we must now retroactively regret our original type assignment and make the hypothesis extractable.
%No big deal: when trying to ``unbox and apply'' 
%No big deal: we substitute the original variable for an $\dep{x}$-marked equivalent (see Figure~\ref{figure:higher_order_nested_dep} to jog your memory) 
...
\begin{figure}[htbp]
	...
	\aethelcap
		...
		{\term{\boxelim{mod}(\con{6}~\diaintro{relcl}((\lam \Var{8}.\boxelim{mod}(\con{9})~(\con{10}~\Var{8}))))~\con{4}}}
