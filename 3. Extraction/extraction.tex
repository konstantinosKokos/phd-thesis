...
All variables that are ``shared'' between conjuncts will be abstracted over. 
The abstraction of variables enacting head- and adjunct- functors will yield proofs of higher-order types.
...
Conjuncts 14 and 24 contain phantom heads 33 and 25 respectively, both coindexed with material node 19.
Abstracting their proofs over the corresponding variables will result in type variable $\chi$ being instantiated as the second order type
$(\ddia{vc}\ww\li\ddia{su}\np\li\smain)\li\smain$, in turn producing a third order type hint for the coordinator... oof.
...
Alternatively, by type raising we could use them to derive corresponding proofs of type $((\ddia{su}\np[s]\otimes\ddia{vc}\ww)\li\smallgtype{a})\li\smallgtype{a}$, for any $\smallgtype{a}$ of our liking.
...
Conjoining a series of arguments and feeding the result to the predicate means that the semantic interpretation of the conjunction would be inescapably bound to a local scope.
Conjoining a series of $\lambda$ abstractions and applying the result to the shared predicate unveils the full context of the conjunction, thus granting the future semantic interpretation the potential to duplicate and distribute meaning if and as desired.
...
%Our hard labor is about done.
%To maintain the homogeneousness of functors regardless of the context they appear in, we must impose an order of abstraction that is reverse to the obliqueness hierarchy we have used so far.
%Head- and adjunct- functors, falling outside the obliqueness order, will have priority over complements, appearing as the first argument of the higher-order types they help form.
%Any secondary coordinator appearing within the same conjunction (now marked as \textit{cor}, due to our earlier transformation), will be the first argument to be consumed by the main coordinator.
...
%					[\mathsf{node}_i \
%%\ \smallgtype{a} \ \_ =  \vari:\smallgtype{a} \ \mathsf{if} \ (\mathsf{phantom} \ \mathsf{node}_i) \ \mathsf{else} \ \con{i}:\smallgtype{a}
%\end{align*}
