...
But a side-effect of this earlier concession is our inability to tell well- and ill- founded ambiguities apart, i.e. we have no means of knowing whether a second proof derivable from the exact same proof frame (sequence of type assignments) is a clear-cut error or indeed a linguistically plausible alternative reading.
Lassy itself does not provide a derivation for each possible reading.
Luckily, it is also internally inconsistent on which reading it prefers.
We can use this to our advantage to try and estimate the \textit{derivational} ambiguity of \AE thel, by first isolating the proof frame of each sample (taking the type assignments independent if what words/phrases they were assigned to), and then counting the amount of unique proofs derived from each frame.%
...
Of the remaining 2\,320, 2\,259 (97.4\%) are mapped to one unique proof, whereas 61 (2.6\%) derive up two unique proofs.
Using this as a guideline, we find 472 samples in the entirety of \AE thel (a measly 0.7\%) that are potentially ambiguous, their frames being among the 61 suspects.
Manual inspection reveals most of them to really be ambiguous (e.g. the proof assigned is one of two possible modifier attachments), with the would-be alternative reading sometimes ruled out on the basis of selectional restrictions.
%due to theory-external constraints (pragmatic or morphological
% manual checking reveals them
%These are mostly really ambiguous (e.g. one of , but sometimes the alternative reading is excluded due to extra-theoretic constraints 
...
Their breakdown is as follows: 2\,405 are behind 1 unique proof, 112 behind 2, 6 behind 3, and 1 is behind 5.
Potentially ambiguous samples are now doubled (909 or 1.7\% of the total), and ambiguities are more commonly artificial, with the would-be alternatives frequently ruled out by morphosyntactic constraints.

What are these boring numbers trying tell us?
Well, two things.
First, that using an undirectional logical core does not cause any horrible explosion of apparent ambiguity: proof frames are sparse, and the bulk of them are pointing to a single proof.
Assigning the correct types will by now means suffice for parsing, but, from a statistical perspective, it will certainly tighten the bound between which proofs are accessible and the one that is linguistically correct.
Second, it tells us that dependency decorations seem to serve an extra role, orthogonal to the one intended.
By restricting the hypotheses launched by higher-order functors to a predetermined grammatical function, they exchange derivational ambiguity for lexical type ambiguity, making up for the lack of 

%Derivational ambiguity is largely subsumed by lexical type ambiguity.
%First, dependency decorations 
...
%But another implication of using $\NLP$ as our logical core is the syncretism between well- and ill- founded ambiguity; i.e. we can't tell whether a second proof arising from the same type assignments is 
