...
\hide[Pragmatically, this means to to try and directly align it with real-world linguistic data at scale.]{To ascertain the model's utility without resorting to promises of theoretical universality, cognitive plausibility, linguistic intrinsicness and the like, we have two tried and tested ways to proceed.
...
Even though main clauses emanate a false sense of safety, coming off as SVO at first glance, Dutch surface word order is SOV canonically, where exceptionally for matrix clauses the verb appears in the \textit{second} position.
...
know you who cheese sell-\abbrv{3sg}\\
...
For starters, bare infinitives usually follow their governor -- but: this is not necessarily the case for clusters of two verbs where the finite verb is a modal.
...
\begin{table}
	...
	\begin{tabularx}{0.925\textwidth}{@{}cccc@{}}
	      ...
	      \textit{ahi} & \textex{aan-het} Infinitive & $<$0.1 & \ahi[s]\\
	      ...
	      \textit{oti} & \textex{om-te} Infinitive & 0.8 & \oti[s]\\
...
A phrasal annotation canonically contains a single head, a collection of complements (no more than one of each), and a collection of adjuncts (without any restriction on their plurality), where verbal, nominal and sentential domains differ in labels they may contain.
...
Discourse level annotations are materialized by dependencies \textit{dlink}, \textit{dpart}, \textit{nucl} and \textit{sat}\hide{, used in a catch-all fashion in place of an actual syntactic analysis}.
...
Multiword expressions are a \hide[inhabitants of the grey area]{pervasive pest from the shadowy realms} between lexicon and syntax.
...
This simple heuristic is as as far as we can get, but it will help homogenize our extracted proofs and types by (i) aligning dominance hierarchy and word order and (ii) alleviating any unecessary typing tension between \smallgtype{np} and \smallgtype{n} modifiers.
...
The \textit{conj} label, used as an umbrella category to classify all conjunctions, carries the same risk as the \textit{mwu} label, namely of \hide[affecting]{polluting} the functional type assignments of phrases outside the conjunction itself with a generic, multi-purpose argument type.
...
Terminal nodes are easy\hide{-peasy}.
...
To see this in practice, let's consider the example of Figure~\ref{figure:vp_ellipsis}.%
	\hide[ ]{\footnote{Give it up for our first ever landscape proof!}}
...
\end{equation*}
in turn producing a fourth order type hint for the coordinator which doesn't even fit in the line...\hide{ oof}.
...
Such a lifted interpretation is aligned in spirit with the long line of work on categorial conjunction semantics~\cite[inter alia]{oehrle1987boolean,hendriks1995ellipsis}, except simplified in \textit{not} having to account for the surface discontinuity caused by the elision of linguistic material.
...
\subsection{Analysis}
\subsubsection{Quantitative Obligations}
...
\paragraph{Theorems}
To quantify the proof-theoretic diversity of the 68\,763 extracted samples, we abstract away from lexical content and end up with a total of 55\,108 unique theorems, where constants are identified indexically (and not by their string).
...
The most common word of \AE thel is the gendered definite determiner \textex{de}, claiming the throne with an incontestible 56\,925 occurrences and making up for 5.7\% of all words just by itself.
...
In (\ref{gloss:whq}), \textex{waarom} both heads the wh-question and provides a hypothetical argument for the verb-initial direct question nested within.
In (\ref{gloss:passive}), the auxiliary \textex{worden} properly accounts for the passive by providing a hypothetical object for the transitive past participle \textex{behandeld}.

\begin{exe}
\ex {\small\fname{dpc-vla-001161-nl-sen.p.107.s.1(1)}}\label{gloss:whq}
...
%	c0 ▵whbody((λx0.case ▿mod(x0) of x1 in ▾mod(x1) (c1 ▵su(c2)))) : WHQ
\glt{$\small\term{
	\con{0}~
	\ddiaintro{whbody}(
		\lam \Var{0}.
			\caseof
				{\ddiaelim{mod}\Var{0}}
				{\Var{1}}
				{(\dboxelim{mod}\Var{1} ~(\con{1}~\ddiaintro{su}\con{2})}
	)
}$}
\glt{`Why do we work?'}
\ex  {\small\fname{WR-P-P-J-0000000001.p.38.s.2(1)}}\label{gloss:passive}
...
\glt{`These will be treated in chapter 4.'}
\end{exe}
...
Examine the type assignments to the relativizer \textex{die} in the examples below, and notice that the hypothesis of (\ref{gloss:su}) is forced to occupy the object slot of \textex{oproepen}, as opposed to the hypothesis of (\ref{gloss:obj}) which is forced to occupy the subject slot of \textex{volgen}.
...
\begin{exe}
\ex {\small\fname{WS-U-E-A-0000000215.p.25.s.1(1)}}\label{gloss:su}
...
\ex  {\small\fname{pc-ind-001645-nl-sen.p.12.s.1(13)}}\label{gloss:obj}
...
\end{exe}
Likewise, types don't have to vary for the two different placements of past participles in the perfect or the passive; inspect the positioning of the auxiliary \textex{heeft} in relation to the past participles \textex{ge\"informeerd} and \textex{bezield} in Examples (\ref{gloss:green}) and (\ref{gloss:red}) below.
...
\ex {\small\fname{WR-P-P-I-0000000173.p.1.s.2(1)}} \label{gloss:red}
...
\begin{exe}
\ex {\small\fname{WS-U-E-A-0000000243.p.1.s.1(14)}}\label{gloss:horror}
...
Thirty women and children\\
	...
	$\con{3}::\np[s]$
\\
\glt{\footnotesize\hfill
	$\chi := \ddia{mod}\dbox{mod}(\np[s]\li\np[s])\li\np[s]$
}
\glt{\footnotesize\hfill
	$\chi^\star := \dbox{mod}(\np[s]\li\np[s])\li\np[s]$
}
       ...
       \term{
       	...
       	&~\ddiaintro{cnj}(
       		...
       		\caseof
       			{\ddiaelim{mod}\Var{2}}
				{\Var{3}}
				{(\dboxelim{mod}\Var{3}~\con{1})}
       	)\\
       	&~\con{0}
    \end{aligned}
  $
}
\glt{`Thirty women and children'}
\end{exe}

\paragraph{Multiwords \& Discourse Parts}
...
in (\ref{gloss:mwp}), for instance, the phrase \textex{Op het laatst} is assigned a single type.
...
\begin{exe}
\ex {\small\fname{WR-P-E-I-0000050381.p.1.s.531(3)}}\label{gloss:mwp}
\glll {Op het laatst} van de oorlog\\
...
The endeavour was popularized by the CCGbank~\cite{10.1162/coli.2007.33.3.355}, a semi-automatically extracted corpus of combinatory categorial grammar derivations, following a proof of concept in German~\cite{hockenmaier2006creating}.
CCGbank has since become a flagship for the applied categorial grammarian, spawning many offsprings, monolingual~\cite[inter alia]{johan2009converting,tse2010chinese,ambati2018hindi}, and aligned~\cite{Bos2017GMB, abzianidze-etal-2017-parallel}.
