...
\end{xlist}
\end{exe}
Interestingly, the language does not make an overt distinction between an object- and a subject- relative pronoun; combined with the SOV word order, and the absence of case markings, the effect is that the two relative clause types end up having the exact same surface form when the grammatical gender of the antecedent noun and the non-gap embedded argument are the same -- contrast the two sentences (\ref{gloss:rc_ambiguity}) below, where the first one is amenable to two different readings.
\begin{exe}
...
%A superficial exposition of some interesting Dutch phenomena will nevertheless follow later on in this chapter's conclusion, where they can be paired with the analyses they are prescribed.
...
Variables and nodes are in a one-to-one relation, owing to their naming scheme; from a variable, we can refer to its node, and from a node we can extract its index, inducing a many-to-one variable to index relation.
...
%With all the numbers laid down, all that's left to do is showcase on how \AE thel treats common and idiosyncratic constructios of Dutch.
...
%
%\paragraph{Infinitives \& Clusters}
%Infinitival forms commonly occur as the verbal complements of a modal, auxiliary or perception verb.
%Depending on the modal, the preposition \texttr{te}{to}, or the discontinuous \texttr{om ... te}{to}, may be either necessary, optionally admissible or completely disallowed.
%Dutch subordinate clauses follow an SOV order, meaning that the chaining of verbs requiring non-finite complements inadvertently leads to \textit{verb clusters}, i.e. collections of two or more verbs situated within the dependent clause and adjacent to one another.
...
%\linebreak\begin{exe}
%\ex
%\nolistbreak\begin{xlist}
%\ex {\small\fname{WS-U-E-A-0000000046.p.8.s.6(1)}}
%\glll Hij erkent dat hij de kamer onjuist ge\"-informeer-d heeft .\\
%He admits that he the chamber wrongly \abbrv{pst.ptcp}-inform-\abbrv{circ} has .\\
%{$\vnw[s]$}
%{$\ddia{vc}\cp[s]\li\ddia{su}\vnw[s]\li\smain[s]$}
%{$\ddia{cmpbody}\ssub[s]\li\cp[s]$}
%{$\vnw[s]$}
%{$\dbox{det}(\n[s]\li\np[s]$}
%{$\n[s]$}
%{$\dbox{mod}(\ppart[s]\li\ppart[s]$}
%{$\ddia{obj1}\np[s]\li\ppart[s]$}
%{$\ddia{vc}\ppart[s]\li\ddia{su}\vnw[s]\li\ssub[s]$}
%{$\punct[s]$}\\
%\glt{\small $\term{
%	\con{1}
