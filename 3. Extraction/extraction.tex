...
\begin{table}
	...
	\begin{tabularx}{0.925\textwidth}{@{}cccc@{}}
	      ...
	      \textcolor{gray}{\textit{du}} & \textcolor{gray}{Discourse Unit} & \textcolor{gray}{2.6} & \textcolor{gray}{n/a}\\
...
\begin{table}
	...
	\begin{tabularx}{0.925\textwidth}{@{}cccc@{}}
	      ...
	      \textcolor{gray}{\textit{dlink}} & \textcolor{gray}{Discourse Link} & \textcolor{gray}{0.2} & \textcolor{gray}{n/a}\\
	      \textcolor{gray}{\textit{dp}} & \textcolor{gray}{Discourse Part} & \textcolor{gray}{0.8} & \textcolor{gray}{n/a}\\
	      ...
	      \textcolor{gray}{\textit{mwu}} & \textcolor{gray}{Multiword Part} & \textcolor{gray}{5.1} & \textcolor{gray}{n/a}\\
	      \textcolor{gray}{\textit{nucl}} & \textcolor{gray}{Nuclear Clause} &\textcolor{gray}{0.5}& \textcolor{gray}{n/a}\\
	      ...
	      \textcolor{gray}{\textit{sat}} & \textcolor{gray}{Satellite} & \textcolor{gray}{0.2} & \textcolor{gray}{n/a}\\
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.6\textwidth}
		\includegraphics[width=1\textwidth,trim={1.25cm 0 1cm 0}]{./prebuilt/date_before.pdf}
	\end{subfigure}%
	\begin{subfigure}[b]{0.4\textwidth}
			\includegraphics[width=1\textwidth,trim={1cm 0 1.25cm 0}]{./prebuilt/date_after.pdf}
	\end{subfigure}\\[\smallsep]
	\lassycap
		{WR-P-P-I-0000000242.p.16.s.2~\textnormal{(excerpt)}}
		{...op 9 en 10 maart...}
		{...on March 9 and 10...}
	\caption{Date expression containing a range.}
	\label{subfigure:mwu:datetime}
	\end{subfigure}\\[\midsep]
	\begin{subfigure}{1\textwidth}
	\begin{subfigure}[b]{0.5\textwidth}
		\includegraphics[width=1\textwidth,trim={1.25cm 0 2cm 0}]{./prebuilt/watvoor_before.pdf}
	\end{subfigure}%
	\begin{subfigure}[b]{0.5\textwidth}
			\includegraphics[width=1\textwidth,trim={2cm 0 1.25cm 0}]{./prebuilt/watvoor_after.pdf}
	\end{subfigure}\\[\smallsep]
	\lassycap
		{WR-P-P-H-0000000039.p.8.s.3(6)}
		{wat voor keuze heb je?}
		{what kind of choice do you have?}
	\caption{Erroneously analyzed interrogative.}
	\label{subfigure:mwu:watvoor}
	\end{subfigure}\\[\midsep]
	\caption{Before (left) and after (right) photos of automatically reannotated multiword expressions.}
	\label{figure:mwu_transformations}
\end{figure}
...
These are also caught and corrected, as in the example of Figure~\ref{subfigure:mwu:watvoor}; the preposition is reannotated as being the head of a prepositional phrase selecting for a direct object, that being the afforementioned ghost node.
...
\begin{figure}
	\begin{subfigure}[b]{0.475\textwidth}
		\includegraphics[width=1\textwidth,trim={1.25cm 0 2.5cm 0}]{./prebuilt/p2crd_before.pdf}
	\end{subfigure}%
	\begin{subfigure}[b]{0.525\textwidth}
			\includegraphics[width=1\textwidth,trim={2.5cm 0 1.25cm 0}]{./prebuilt/p2crd_after.pdf}
	\end{subfigure}\\[\smallsep]
	\lassycap
...
\begin{figure}
	\begin{subfigure}[b]{0.55\textwidth}
		\includegraphics[width=1\textwidth, trim={1.5cm 0 1.5cm 0}]{./prebuilt/ua_before.pdf}
	\end{subfigure}%
	\begin{subfigure}[b]{0.45\textwidth}
			\includegraphics[width=1\textwidth,trim={1.5cm 0 1.5cm 0}]{./prebuilt/ua_after.pdf}
	\end{subfigure}\\[\smallsep]
	\lassycap
...
\begin{figure}
	\begin{subfigure}[b]{0.523\textwidth}
		\includegraphics[width=1\textwidth, trim={1.5cm 0 3cm 0}]{./prebuilt/prenom_before.pdf}
	\end{subfigure}%
	\begin{subfigure}[b]{0.477\textwidth}
			\includegraphics[width=1\textwidth,trim={3cm 0 1.5cm 0}]{./prebuilt/prenom_after.pdf}
	\end{subfigure}\\[\smallsep]
	\lassycap
...
This allows an easy adaptation to morpologically informed types, or a transition to an expanded theory (e.g. one that includes subtyping, additional/other axes of modal decorations, etc.). 
...
In any case, the translation induces a function $\mathsf{translate} :: \mathsf{Node} \to \mathsf{Type}$ that naively tells us for each node what type the translation table prescribes to its part of speech tag (if lexical), syntactic category (if phrasal) or the correspond translations of its material counterpart (if ghost).

Then, we need an equivalence relation on the set of dependencies so as to partition it into heads, adjuncts $\adjuncts$ and complements $\complements$ -- each non-head dependency is translated into a modal label, the union of which (together with the extraction modality) forming the set of modalities $\deps$.
...
In mathy font, we have:
\begin{align*}
\mathsf{prove} :: \mathsf{Tree} \to \mathsf{Type}^? \to {\deps}^? \to \mathsf{Proof}
\end{align*}
In the paragraphs to follow, we will inspect the tree structures most commonly encountered and discuss their treatment in high level terms (plus some language agnostic but serious looking pseudocode).
...
If it was a ghost, we must recall our discussion in Section~\ref{subsubsec:sreason_dep} (hypotheses come prepackaged with their modalities, you're welcome) and check whether a modality needs to be added -- a diamond (if we got here through a complement marking dependency), or a box (an adjunct marking one).
...
\mathsf{and}\  \gtype{b} & \mapsfrom \ddia{dep}\smallgtype{a} \ \mathsf{if} \  \dep{dep} \in \complements \ \mathsf{else}
...
If a type hint was passed from above, we have no choice but to obey the oracle.
If no hint was passed, we will translate the phrasal category of the current root into the appropriate top type.


\begin{figure}
	\includegraphics[width=1\textwidth, trim={1.5cm 0 1.5cm 0}]{./prebuilt/simple_extraction.pdf}
	\aethelcap
		{WR-P-E-I-0000041235.p.1.s.123(1)}
		{Al het andere is afgeleid.}
		{All the rest is derivative.}
		{\smallterm{\con{7}~\diaintro{vc}(\con{10})}}
	\caption{Simple finite clause extraction.}
	\label{figure:simple_extraction}
%	c7, 〈c10〉vc, 〈〈〈c4〉mod, c5〉mod, c6〉su ⊢ c7 ▵vc(c10) ▵su(▾mod(▾mod(c4) c5) c6) : SMAIN
\end{figure}


\paragraph{Simple Clauses}
...
% this order does not apply to N->N
...
%From each proof we get back, we will inspect all hypotheses contained therein -- if any of these hypotheses shares the same index as the head of the clause, we will abstract over it.
...
The way to go is simple: we need to left fold the head proof against its arguments, and then right fold the ``unboxed'' adjuncts to the result, careful to follow the correct order.
...
Now, if your senses are telling you this looks too easy to be true, you're right.
This simple setup already suffices to cover quite a lot of trees
...
%This functionality is encapsulated by the $\mathsf{make\_args}$ function.
%\begin{align*} 
%& \mathsf{make\_args} :: [(\dep{dep},\mathsf{Tree})] \to \mathsf{Node} \to \mathsf{Type}^? \to [\mathsf{Proof}]\\
%& \mathsf{make\_args} \ \mathsf{comps} \ \mathsf{head} \ \mathsf{type^?} = \\
%where & \mathsf{proofs} \ & \mapsfrom \ [\mathsf{prove} \ \mathsf{t}_i \ \mathsf{type}^? \ \mathsf{d}_i \ | \ (\mathsf{d}_i,\mathsf{t}_i) \in \mathsf{comps} ]
%%& \mathsf{as} \ \mapsfrom 
%\end{align*}
