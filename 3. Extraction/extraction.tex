...
In (\ref{gloss:horror}) our defense of Lassy's tendency to distribute modifiers among conjuncts backfires, as we end up with a coordinator joining not two noun phrases, but noun phrases missing a mobile modifier. 
...
Thirty women and children\\
	$\con{0}::\dbox{mod}(\np[s]\li\np[s])$
	$\con{1}::\np[s]$
	$\con{2}::\ddia{cnj}{\chi}\li\ddia{cnj}(\chi)\li\chi$
	$\con{3}::\np[s]$
%	\li
%	\ddia{mod}\dbox{mod}(\np[s\li\np[s])\li\np[s]$
%	$\np[s]$
