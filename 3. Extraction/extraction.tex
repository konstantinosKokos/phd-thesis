...
On the codomain's side, we have a grand total of 6\,008 unique \NLPplus{} types, which, stripped of their modalities, fall back to 3\,641 pure linear types.
...
Despite most lexical entries (56\,176) having a single unique assignment, ambiguities are quite common, the number of assignments exponentially\textsubscript{2} increasing for a logarthmically\textsubscript{10} declining number of entries.
At the end of the tail we find several chameleon words appearing in multiple guises; chief among them is the usual suspect \texttr{en}{and}, counting as many as 1\,046 (!) unique types in just 41\,614 occurrences -- the price due for our choice to brave conjunctions with concretely instanced, pre-raised types.
But this is painting a perhaps overly grim picture.
Seen as random variables, lexical entries actually have most of their probability masses concentrated on their modes; the average probability of the most common type per entry lies at a (very high) 88.97\%. 
Naively assigning the mode to each lexical assignment corpus-wide (regardless of any context) establishes a comfortable 66.97\% accuracy baseline, asserting that our assignments are in practice quite consistent within their entries.
...
Figure~\ref{figure:entry_sparsity} shows the distribution of lexical entry occurrences; most entries occur just once, whereas a handful occur up to a few thousand times (interpolate between most and a handful in the log\textsubscript{10}/log\textsubscript{2} plane to find the any point of interest).
