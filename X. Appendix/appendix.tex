...
Implementing a type system in an untyped language is a perversity of nature, but allows at least a subversive title, and, with it, a chance to a medium-hosted blog post.
More than providing comic value, it makes the code easy to integrate with standard machine learning libraries, for which Python is the de facto choice -- we're just planning ahead.
...
\noindent which we can then wrap into a \py{Query} and compose it with a type condition to find the longest sentence with strictly alphanumerically descending initial word letters:
