...
What if there was a way to keep our logic computationally tractable but with temporary\hide[,]{ and} on-demand access to normally excluded reasoning tools?
...
In our case, we will make do with two modalities from temporal logic, the diamond $\diamond$ and the box $\bx$.%
	\footnote{Note that these are not duals, but \textit{inverse} duals -- $\diamond$ being past necessity, $\bx$ being future possibility.}
...
% said to recite daily before bedtime:
\begin{quote}
