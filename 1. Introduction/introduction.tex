...
\chapabstract{In the beginning was the word,\\
and the word had a \type.}
...
Given some finite set of \textit{atomic formulas} \propcon{}, and \prop{a}, \prop{b}, \prop{c} arbitrary well-formed propositions, the language of propositions in Backus-Naur form is inductively defined as:
\begin{equation}
...
$\Gamma \vdash \prop{a}$.
We read such a statement as a suggestion that from \textit{assumptions} $\Gamma$ (i.e. a structure of \textit{hypotheses}) one can derive the proposition \smallprop{a}.
...
First, the exchange rule $\Exchange$ explicates an extra property of our structure binding operator, namely commutativity.
...
\begin{figure}
	...
	\begin{subfigure}{1\textwidth}
		...
		\begin{tabularx}{0.7\textwidth}{@{}C@{\qquad}C@{}}
		\multicolumn{2}{@{}c@{}}{
			$\infer[\Exchange{}]{\Gamma, \prop{a}, \prop{b}, \Delta \vdash \prop{c}}{\Gamma, \prop{b}, \prop{a}, \Delta \vdash \prop{c}}$}\\[\smallsep]
...
\begin{figure}
	...
	\begin{tabularx}{0.975\textwidth}{@{}C@{~}C@{}}
		...
		\multicolumn{2}{@{}c@{}}{$\infer[\Exchange{}]{\Gamma, \prop{a}, \prop{b}, \Delta \vdash \term{s}: \prop{c}}{\Gamma, \prop{b}, \prop{a}, \Delta \vdash \term{s}: \prop{c}}$}\\[2em]
		$\infer[\Weakening{}]
...
With $\Contraction{}$ we can pretend to be using two different instances $\vari$ and $\varj$ of the same type before identifying the two as a single object $\vark$ in term \term{s};
note here the meta-notation for \textit{variable substitution}, $\smallterm{s}_{[\smallterm{\vari } \mapsto \smallterm{t}]}$, which reads as ``replace any occurrence of variable $\vari$ with term \smallterm{t}.
...
\paragraph{$\alpha$ conversion} is a semantically null rewrite obtained by renaming variables according to the substitution meta-notation ${[\smallterm{\vari } \mapsto \smallterm{\varj}]}$ described above. 
...
Figure~\ref{figure:CHC} summarizes the subsection.
...
It states that for \smallprop{a}, \smallprop{b} arbitrary propositions, from \smallprop{a} one can derive its raised form $(\smallprop{a} \ii \smallprop{b})\ii\smallprop{b}$.
...
For the purposes of this thesis, we will focus our presentation on linear type theory.
...
\begin{figure}
	...
	\begin{subfigure}{1\textwidth}
		...
		\draw[dashed] (li3) -- (A2) node[midway, left]{$\term{\vark}$};
		\draw (li3) -- (C2);
		\draw[->, dotted, thick] (A1) -- ($(A1.north) + (0, 0.5)$) -- ($(A2.north) + (0, 0.5)$) -- (A2);
		\draw[->, dotted, thick] (B2) -- ($(B2.north) + (0, 0.25)$) -- ($(B1.north) + (0, 0.25)$) -- (B1);
		\draw[->, dotted, thick] (C2) -- ($(C2.north) + (0, 0.75)$) -- ($(C1.north) + (0, 0.75)$) -- (C1);
	\end{tikzpicture}
...
Whichever the case, we cannot reasonably anticipate there to always be a single possible set of axiom links, so we need a decision procedure that tells us how to actually extract them from a less forgiving proof.

\begin{figure}
	\centering
	\begin{tikzpicture}[
		   focus/.style={circle, draw, minimum size=17pt},
	...
	\node[focus] (am) at (3.7	,  1.2) {};
	\draw[link, <-] (ak) to[bend left=45] (am); %$(am.north)$);
	\node[focus] (bl1) at	(1.5, 1.2) {};
	\node[focus] (bl2) at (2.45, 0.6) {};
	\draw[link, ->] (bl1) -- (bl2);
	\node[focus] (bi) at (-3.35, 0.6) {};
	\node[focus] (cj1) at (-2.45, 0.6) {};
	\node[focus] (cj2) at (0.6, -0.05) {};	
...
\paragraph{Proof Nets and Search}
...
The logic that provides us with the tools to accomplish this is due to Jim Lambek~\cite{lambek1958mathematics}, and has come to be known as the Lambek calculus $\LC$.
...
In the case for checking the satisfiability of a judgement (i.e. searching for \textit{any} valid proofs), all the above would have made for potential proof candidates;
...
In such a scenario, taking a step up in the hierarchy would cause an undesirable combinatorial explosion, whereas staying put would sacrifice our ability to argue about these \hide{exceptional } cases. 
...
In our case, we will make do with two modalities from temporal logic, the diamond $\diamond$ and the box $\bx$.%
	\footnote{Note that these are not the necessity/possibility duals of modal logic, but rather \textit{inverse} duals, $\bx$ being \textit{past} necessity and $\diamond$ being \textit{future} possibility. Read (\ref{equation:closure}) as ``what is, has always been bound to be'' and (\ref{equation:interior}) as ``what will have always been, is''.}
...
Type-logical grammars extend the Curry-Howard correspondence with a new axis, that of natural language; the transference of points of interest across that axis is presented in Figure~\ref{figure:CHC-lang}; our motto shall from now on be \textit{parsing as deduction}.
...
%		\draw node[rectangle, minimum width=50pt,draw=black, minimum height=110pt,dotted,thick,label={$\Gamma$}] (x) at (-0.1,-1.5) {};
		...
		\end{tikzpicture}
		&
		\raisebox{30pt}{$\xleftarrow{\rulestyle{ass}_{\diamond}}$}
		&
		\begin{tikzpicture}
%		\draw node[rectangle, minimum width=50pt,draw=black, minimum height=110pt,dotted,thick,label={$\Gamma$}] (x) at (0.1,-1.5) {};
		\Tree 
...
%		\draw node[rectangle, minimum width=50pt,draw=black, minimum height=110pt,dotted,thick,label={$\Gamma$}] (x) at (-0.1,-1.5) {};
		...
		\end{tikzpicture}
		&
		\raisebox{30pt}{$\xleftarrow{\rulestyle{mix}_{\diamond}}$}
		&
		\begin{tikzpicture}
%		\draw node[rectangle, minimum width=50pt,draw=black, minimum height=110pt,dotted,thick,label={$\Gamma$}] (x) at (-0.1,-1.5) {};
		\Tree
...
Let's make things unnecessarily convoluted for the sake of clich{\'e}d self-referentialism by considering the relative clause \textex{which can vary in its position} of the previous sentence.
...
\hide[commonly heard reciting the quote]{said to recite daily before bedtime}:
\begin{quote}
...
The only constraint imposed is the only one that matters: the high-level principle of compositionality needs to hold, i.e. the function-argument structure specified by syntax needs to be carried through to semantics.
...
Note also that such a path is merely a suggestion and not an imperative; a more ambitious line of thought could maintain that word order variations (and the structural rules licensing them) can carry semantic cues which, albeit subtle, need to be upheld in the compositional meaning translation; see for instance  the contemporary work of \citet{duarte2022quantum} for a quantum (!) interpretation of the control modalities.

\paragraph{The Role of the Lexicon}
The sentiments of the previous paragraph could be met with some skepticism.
A reader with a critical eye might argue that semantic interactions not already manifested in the syntax may never be born of this process, and thus wonder whether this added expressivity can serve any real purpose or offer any tangible benefits.
To dispel such doubts, we need to keep in mind that derivational terms refrain from specifying \textit{lexical meaning}, i.e. they treat lexical items as semantic black boxes.
...
\begin{figure}
	\centering
	\begin{tikzpicture}
		\node (ds)		at (0, 2)		{Tectogrammar};
		\node (dsl)		at (0, 1.5)		{$\Sigma := \ILL_{\li}$};
		\node (ss)		at (-4.5, 0)		{Phenogrammar};
...
and the maximal order of the translation is 2, making our grammar's complexity class (1, 2), the subset that encapsulates context-free grammars.%
	\footnote{Or (2,2) if we set sail from $\mathcal{O}(p) = 1$ for the base case $p \in \propcon$ in (\ref{equation:type_order}), as commonly done in abstract categorial grammar literature.}
...
Further away from the radiant warmth of type theory, we find a deviant from the categorial tradition in combinatory categorial grammars~\cite{ades1982order,szabolcsi1989bound, steedman2022combinatory}.
