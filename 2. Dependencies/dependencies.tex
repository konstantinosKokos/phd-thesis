...
Equipped with this missing bit of \hide{alchemical }understanding, we're at long last able to produce analyses for some less contrived linguistic examples; Figure~\ref{figure:lovecraft_dep_rep_clause} presents the dependency-enhanced derivation we set out to deliver.
...
We will need to once more resort to the $\diamond E$ rule to remove , except this time it will be the interior combination of Figure~\ref{subfigure:modal_properties:interior} that we are invoking, which is not subject to $\eta$ contraction.
...
\textit{Monads} make for a natural translation choice, both because of their affinity with modalities~\cite{kobayashi1997monad} as well as their progressively maturing adoption by the linguistics community~\cite{asudeh2020enriched}.
...
On a relevant note, the slightly disheveled categorial dependency grammars make for an attempt to provide a categorial treatment of dependency structure, where the arcs and their labels are themselves the grammar's base types~\cite{dekhtyar2015categorial}.
