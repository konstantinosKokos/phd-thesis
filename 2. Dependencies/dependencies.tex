...
This observation gives rise to a binary subcategorization of a binary predicate structure; in dependency grammar terms, it is either:
\begin{enumerate}
	\item an application of a head to its \textit{complement}, or
	\item an applicaiton of an \textit{adjunct} to its head
\end{enumerate}
...
The authors refrain from commiting to a specific linguistic application, but, translated into our dependency grammar terminology, their proposal can be schematically summarized by Figure~\ref{figure:heads_and_phrases}.
...
As an alternative to introducing implicational (and by residuation, product) variants, we can instead opt for a modal decomposition~\cite{kurtonina1997structural}.
In this approach, a uniform implicational operator would combine with a modal marking to produce the specialized (here: head-aware) variant.
The standard route would have us mark the head -- we will instead mark the dependent.
...
Starting with the former, such a decomposition would look as follows:
\begin{align}
	\prop{b} \divright_l \prop{a} \ \equiv \ \prop{b} \divright \diamond \prop{a} \\
	\prop{a} \divleft_r \prop{b} \ \equiv \ \diamond  \prop{a} \divleft \prop{b}
\end{align}
The translation is straightforward: functors in head position will only consume their complements after having \textit{assigned} them a diamond type (i.e. via the $\diamond I$ rule of Figure~\ref{figure:modal_logical}).
%In the latter case things are slightly more obscure; now the entire functor corresponds to the dependent, therefore adjuncts will need to \textit{project} their dependencies 
%there, we need to follow the dual path, putting the entire functor under the scope of a box:
%\begin{align}
%	\prop{b} \divright_r \prop{a} \ \equiv \ \Box(\prop{b} \divright \prop{a}) \\
%	\prop{a} \divleft_l \prop{b} \ \equiv \ \Box (  \prop{a} \divleft \prop{b})
%\end{align}

%For the time being, let's inspect the effect this decoration would have to a proof.
%The effect of this is an elegant and universal treatment of dependency demarcation that finds its way both to the antecedent structure of a judgement, and its term imprint;
