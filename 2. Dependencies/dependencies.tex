...
Atomic type assignments will remain for the most part unchanged, as propositional constants are necessarily complements (or heads of a singleton phrase, to be pedantic) and their grammatical role cannot be decided \textit{a priori}, anticipating a phrasal head to enforce it instead.
...
\section{\AE thel}
With our theorycrafting over, we have in our hands a descriptive model of syntactic and semantic composition, promising to capture dependency relations while keeping both its feet set firmly in type theory.
...
What is undisputable, however, is the \textit{omniversality} of the $\lambda$ calculus~\cite{wadler2015propositions} -- we're at least certain that our modeling tools are not some ad hoc machinery designed to tackle a highly localized problem, but the one and only programming language ever worth writing. 
Beyond our tooling, and having \textit{a priori} given up any ideation of truth, the only measure of success for our model is that of its utility (``...but some are useful'', ~\cite{doi:10.1080/01621459.1976.10480949}).
This sets up a new research imperative: we must prove the model useful!

``Oof, that's a hard one'', you might say, and you'll be right.
Thankfully, there are two tried and tested ways to proceed. 
The traditional way involves fiercely advocating the idea, bashing the competition at workshops and conferences, making bold claims and generating traction at any given chance, going all out on grant applications, eventually building a cult of personality, and finally resting on your laurels as the hype becomes self-sustainining and you find yourself being cited for papers you have contributed little to other than your name; at long last, utility affirmed by popular approval.
This first methodology, sometimes called the \textit{scientific method}, is a rather involved and painstakingly long process that starts yielding profits in the long run.
The modern way, more befitting the temporary and precarious contract of the mobile, adaptive, multi-purpose academic laborer, is a short-term investment: simply do everything you can do yourself within the timespan you're given and hope for the best.
We'll go for this one.

This choice has some methodological repercussions.
%Rather than let the theory be disseminated, criticized, adapted, error-corrected and returned to sender, we'll have to put it straight to the test.
